...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
...
export default function Home() {
  ...
  const [continuousListening, setContinuousListening] = useState(false);
  const [isListening, setIsListening] = useState(false);
  const [fallingMessages, setFallingMessages] = useState<<Set<string>>(new Set());
  const mediaRecorderRef = useRef<<MediaRecorder | null>(null);
  const chunksRef = useRef<<Blob[]>([]);
  const messagesEndRef = useRef<<HTMLDivElement>(null);
  const streamRef = useRef<<MediaStream | null>(null);
  ...
  const continuousListeningRef = useRef(continuousListening);
  const messagesRef = useRef(messages);
  const fallingMessagesRef = useRef(fallingMessages);
  ...
  useEffect(() => {
    fallingMessagesRef.current = fallingMessages;
  }, [fallingMessages]);

  // Handle falling rain animation after 5 seconds
  useEffect(() => {
    const timers: NodeJS.Timeout[] = [];
    
    messages.forEach((message) => {
      // Skip system messages and messages already set to fall
      if (message.role !== 'system' && !fallingMessagesRef.current.has(message.id)) {
        const timer = setTimeout(() => {
          setFallingMessages((prev) => new Set([...prev, message.id]));
        }, 5000);
        timers.push(timer);
      }
    });
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <style jsx>{`
        @keyframes fallAndScatter {
          ...
          15% {
            transform: translateY(30px) translateX(-8px) rotate(-3deg) scale(0.98);
            opacity: 0.95;
          }
          30% {
            transform: translateY(80px) translateX(12px) rotate(5deg) scale(0.95);
            opacity: 0.85;
          }
          45% {
            transform: translateY(150px) translateX(-18px) rotate(-8deg) scale(0.9);
            opacity: 0.7;
          }
          60% {
            transform: translateY(240px) translateX(25px) rotate(12deg) scale(0.85);
            opacity: 0.5;
          }
          75% {
            transform: translateY(350px) translateX(-30px) rotate(-15deg) scale(0.8);
            opacity: 0.25;
          }
          90% {
            transform: translateY(500px) translateX(40px) rotate(20deg) scale(0.75);
            opacity: 0.1;
          }
          100% {
            transform: translateY(700px) translateX(-50px) rotate(25deg) scale(0.7);
            opacity: 0;
          }
        }
        .falling-rain {
          animation: fallAndScatter 3s ease-in forwards;
          pointer-events: none;
          z-index: 0;
        }
      `}</style>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-white">
              {messages.slice(1).map((message) => (
                <div
                  ...
                  className={`flex items-start space-x-2 ${
                    message.role === 'user' ? 'justify-end' : 'justify-start'
                  } ${fallingMessages.has(message.id) ? 'falling-rain' : ''}`}
                >
                  {message.role === 'assistant' && (
                    <div className="w-8 h-8 border-2 border-black bg-white flex items-center justify-center flex-shrink-0">
                      <Bot size={18} className="text-black" />
