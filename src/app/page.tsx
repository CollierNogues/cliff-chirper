...
// CUSTOMIZATION: Replace these with your actual filenames
const BACKGROUND_IMAGE = 'your-image-name.jpg'; // <-- CHANGE THIS
const MP3_FILE = 'your-audio-file.mp3'; // <-- CHANGE THIS to your mp3 filename
// ============================================
...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      audio.onended = () => {
        // Will loop automatically since loop is true
      };
  ...
  // Function to play/pause ambient audio
    ...
    if (audioRef.current) {
      if (isAudioPlaying) {
        ...
        audioRef.current.play().catch(err => {
          ...
          alert('Could not play audio. Please check the file format and try again.');
        });
      }
    } else {
      alert('Audio file not found. Please make sure the MP3 file is in the public folder.');
    }
  };

  // Function to dismiss the audio prompt
  ...
  const speakText = async (text: string) => {
    try {
      ...
      speechAudio.onended = () => {
        console.log('Speech audio playback ended');
        setIsSpeaking(false);
    ...
    >
      ...
      {showAudioPrompt && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-sm">
          <div className="bg-black/80 border-2 border-white/30 p-6 max-w-md mx-4 rounded-lg backdrop-blur-md">
            <h2 className="text-xl font-bold mb-4 text-white">Listen to the River Oracle?</h2>
            <p className="text-white/80 mb-6">
              Would you like to hear the ambient audio while you chat?
            </p>
            <div className="flex gap-4 justify-end">
              <button
                onClick={() => {
                  dismissAudioPrompt();
                  // Auto-play audio if they want to hear it
                  setTimeout(() => {
                    if (audioRef.current) {
                ...
                className="px-4 py-2 bg-white text-black border-2 border-white hover:opacity-80 transition-opacity font-mono text-sm"
              >
                YES, PLAY AUDIO
              </button>
              <button
                onClick={dismissAudioPrompt}
                className="px-4 py-2 bg-transparent text-white border-2 border-white/50 hover:bg-white/10 transition-colors font-mono text-sm"
              >
                NO, THANKS
              </button>
            </div>
      ...
      {/* Ambient Audio Control - appears after prompt is dismissed */}
      {!showAudioPrompt && (
        <div className="fixed bottom-4 right-4 z-20">
          <button
            ...
            title={isAudioPlaying ? 'Pause ambient audio' : 'Play ambient audio'}
          >
                ...
                >
                  ...
                  {message.role === 'assistant' && (
                    <button
                      ...
                      className="mt-1 text-white/80 hover:text-white transition-opacity border border-white/30 px-2 py-1 bg-black/30 backdrop-blur-sm text-xs font-mono"
          ...
          <div className="p-3 border-t-4 border-white/20 bg-black/30 backdrop-blur-sm">
            <form onSubmit={handleSubmit} className="flex items-center space-x-2">
              <input
                ...
                className={`flex-1 p-2 border-2 focus:outline-none transition-all text-sm backdrop-blur-sm ${
                  'bg-black/40 text-white border-white/30 placeholder-white/50'
                }`}
                style={{ fontFamily: '"Times New Roman", Times, serif' }}
                disabled={isLoading}
              />
...
export default function Home() {
  ...
  return (
    ...
    >
      {/* Semi-transparent overlay for better text readability (optional - remove if you want the image clear) */}
      <div className="absolute inset-0 bg-black/40 pointer-events-none" style={{ zIndex: 0 }}></div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-3">
                ...
                {isListening && !isSpeaking && (
                  <span className="text-xs text-white flex items-center space-x-1 border border-white px-2 py-1 font-mono bg-black/30 backdrop-blur-sm">
                ...
                {continuousListening && !isListening && !isSpeaking && (
                  <span className="text-xs text-white border border-white px-2 py-1 font-mono bg-black/30 backdrop-blur-sm">
                    PAUSED
                  </span>
                )}
              </div>
